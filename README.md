...
if (listpage.querySelector('rwc-tab[title="Saved Searches"][active]') !== null || listpage.querySelector('a.browser.browser-active[title="Saved Searches"]') !== null) {
	...
	if (searches.length == 0) {
					...
					fetch('https://' + host + '/Relativity.Rest/API/Relativity.Services.Search.ISearchModule/Analytics%20Search%20Manager/ReadSingleAsync', {
						...
						if (JSON.stringify(jsn) == '{"ErrorType":"Relativity.Services.Exceptions.ServiceException","Identifier":"","Message":"Artifact matching that Artifact ID not found."}') {
							fetch('https://' + host + '/Relativity.Rest/API/Relativity.Services.Search.ISearchModule/dtSearch%20Manager/ReadSingleAsync	', {
								...
								searchjsons.push(jsn);
								if (searchjsons.length == searches.length) {
									downloadCSV();
								}
							});
						} else {
		...
		function downloadCSV() {
			...
			dlink.setAttribute('download', 'SavedSearches.csv');
			dlink.click();
		}
	}
}
...
You run Name Normalization and suddenly the "All Custodians" field is polluted with tens of thousands of non-custodian entities. Run this bookmarklet when in the Edit Search interface, and it will filter out all Analytics Entities when performing condition searches.
```js
let csrftoken = window.top.GetCsrfTokenFromPage();
let host = window.top.location.host;
let urlparams = new URLSearchParams(window.top.location.search);
...
fetch('https://' + host + '/Relativity.Rest/API/Relativity.Objects/workspace/' + workspaceid.toString() + '/object/queryslim', {
	...
	fetch('https://' + host + '/Relativity.Rest/API/Relativity.Objects/workspace/' + workspaceid.toString() + '/object/queryslim', {
		...
		try {
			listpage.document.querySelector('rwc-modal-layout.auto-width').querySelector('span[slot="actions"]').querySelectorAll('button')[0].addEventListener('click', function(event) {
				listpage.XMLHttpRequest.prototype.send = listpage.XMLHttpRequest.prototype.nativeSend;
			});
			listpage.document.querySelector('rwc-modal-layout.auto-width').querySelector('span[slot="actions"]').querySelectorAll('button')[1].addEventListener('click', function(event) {
				listpage.XMLHttpRequest.prototype.send = listpage.XMLHttpRequest.prototype.nativeSend;
			});
			listpage.document.querySelector('rwc-modal-layout.auto-width').querySelector('span[slot="actions"]').querySelectorAll('button')[2].addEventListener('click', function(event) {
				listpage.XMLHttpRequest.prototype.send = listpage.XMLHttpRequest.prototype.nativeSend;
			});
			listpage.document.querySelector('rwc-modal-layout.auto-width').querySelector('span[slot="actions"]').querySelectorAll('button')[3].addEventListener('click', function(event) {
				listpage.XMLHttpRequest.prototype.send = listpage.XMLHttpRequest.prototype.nativeSend;
			});
		} catch {}
		try {
			listpage.document.querySelector('div.saved-search-viewer').querySelector('div.modal-footer').querySelectorAll('button.small')[0].addEventListener('click', function(event) {
				listpage.XMLHttpRequest.prototype.send = listpage.XMLHttpRequest.prototype.nativeSend;
			});
			listpage.document.querySelector('div.saved-search-viewer').querySelector('div.modal-footer').querySelectorAll('button.small')[1].addEventListener('click', function(event) {
				listpage.XMLHttpRequest.prototype.send = listpage.XMLHttpRequest.prototype.nativeSend;
			});
			listpage.document.querySelector('div.saved-search-viewer').querySelector('div.modal-footer').querySelectorAll('button.small')[2].addEventListener('click', function(event) {
				listpage.XMLHttpRequest.prototype.send = listpage.XMLHttpRequest.prototype.nativeSend;
			});
			listpage.document.querySelector('div.saved-search-viewer').querySelector('div.modal-footer').querySelectorAll('button.small')[3].addEventListener('click', function(event) {
				listpage.XMLHttpRequest.prototype.send = listpage.XMLHttpRequest.prototype.nativeSend;
			});
		} catch {}
	});
